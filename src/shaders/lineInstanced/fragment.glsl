...
float drawBrush(vec2 uv, float time, float strength, float seed, float ratio) {
	...
	float r = time * speed;
	float fade = (1.0 - smoothstep(delay + r, delay + r + 0.02, uv.y)) * smoothstep(1.0, life, uv.y);
	// fade = 1.0;
	// fade = smoothstep(1.0, life, uv.y);
...
void main() {
	...
	float paper = texture2D(uPaperTex, vUv).r;
