...
void main() {
	...
	float t1 = mix(0.0, 1.0, smoothstep(0.0, 0.2, uRatio));
	float t2 = mix(0.0, 1.0, smoothstep(0.8, 1.0, uRatio));

	float fade = (1.0 - smoothstep(t1, t1 + 0.05, vUv.y)) * (smoothstep(t2, t2 + 0.05, vUv.y));
